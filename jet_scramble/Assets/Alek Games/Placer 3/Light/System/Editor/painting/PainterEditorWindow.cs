    ...
    public class PainterEditorWindow : EditorWindow, IPrefabPalleteUser
    {
        ...
        private void OnGUI()
        {
            ...
            if (helpWindows)
            {
                ...
                if ((isPlacingMode() ? lastV.holdActivation :
            ...
            if (isPlacingMode())
            {
                ...
                lastV.holdActivation = EditorGUILayout.Toggle(new GUIContent("Hold Activation", ""), lastV.holdActivation);

                if (lastV.holdActivation)
                {
                    if (helpWindows)
                    ...
                    if (lastV.holdActivationDistance < 1) lastV.holdActivationDistance = 1;
                }
        ...
        void beforeScene(SceneView scene)
        {
            ...
            if (e.isMouse)
            {
                ...
                if (!rayHit)
                {
                    ...
                    else brushNote += "not pointing at any object on the ground layer\n";
                }
                else if (obstacleDetected) brushNote += " detected an obstacle on the avoided layer\n";
                ...
                if (e.button == 0)
                {
                    ...
                    else if (((e.type == EventType.MouseDown) || continousPlacing()))
                    {
                        ...
                        if (lastSpawn != null && Vector3.Distance(lastSpawn.Value, paintPos) < pValues.holdActivationDistance) return;
                        ...
                        if (pValues.holdActivation && isPlacingMode())
                            lastSpawn = paintPos;
        ...
        {
            return isPlacing && (pValues.holdActivation || !isPlacingMode());
        }
        ...
        {
            //GameObject spawned = 
            pValues.palette.spawn(pos, surfaceNormal, transform, YRotationOffset, pValues.specificIndex);
        ...
        private bool setPlacePos(RaycastHit info, LayerMask ground, LayerMask avoided)
        {
            if (usingGrid() && isPlacingMode())
            {
                ...
                if (usingPlaneProjection() && !planeIsUp())
                {
                    ...
                    //planeProjectionRayCast(new Ray(gridedPos + Vector3.up * pValues.placeGridSize, Vector3.down), out RaycastHit h);
                    //gridedPos = h.point;
                }
                else doStufToPos(ref gridedPos);
